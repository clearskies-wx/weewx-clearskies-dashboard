...
export function BoatingTab({ locationId, alerts = [] }: BoatingTabProps) {
  ...
  return (
    <div className="flex flex-col gap-[var(--gap-grid)]">
      ...
      {/* 4. Live buoy observations */}
      <Panel title={t('boating.buoyObservations')}>
        {observation ? (
          <>
            <p className="text-muted-foreground" style={{ fontSize: 'var(--text-label)' }}>
              {t('boating.station', { id: observation.stationId })}
              {updatedLabel && ` · ${t('lastUpdated', { time: updatedLabel })}`}
            </p>
            <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-3">
              <StatTile
                label={t('windSpeed')}
                value={formatValue(observation.windSpeed, 'wind', locale)}
                unit={windUnit}
              />
              <StatTile
                label={t('waveHeight')}
              ...
              <StatTile
                ...
                value={formatValue(observation.pressure, 'barometer', locale)}
                unit={pressureUnit}
              />
              <StatTile
                label={t('airTemp')}
              ...
              <StatTile
                ...
                value={formatValue(observation.visibility, 'default', locale)}
                unit={visibilityUnit}
              />
            </dl>
          </>
      ...
      <Panel title={t('boating.pressure')}>
        <div className="flex items-center gap-3 flex-wrap">
          <span
            className="text-foreground font-semibold"
            style={{ fontSize: 'var(--text-stat-tile)', fontFeatureSettings: '"tnum"' }}
          >
            {formatValue(observation?.pressure ?? null, 'barometer', locale)}
            <span className="text-muted-foreground font-normal ml-1" style={{ fontSize: 'var(--text-label)' }}>
              {pressureUnit}
            </span>
          </span>
      ...
      {/* 6. Visibility indicator */}
      <Panel title={t('boating.visibility')}>
        <p
          className="text-foreground font-semibold"
          style={{ fontSize: 'var(--text-stat-tile)', fontFeatureSettings: '"tnum"' }}
        >
          {formatValue(observation?.visibility ?? null, 'default', locale)}
          <span className="text-muted-foreground font-normal ml-1" style={{ fontSize: 'var(--text-label)' }}>
            {visibilityUnit}
          </span>
        </p>
      ...
      {/* 8. NWS marine text forecast — expandable sections */}
