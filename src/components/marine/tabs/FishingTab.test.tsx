...
vi.mock('../../../hooks/useWeatherData', () => ({
  useFishingDetail: () => ({ data: { locationName: 'Harbour', coordinates: { lat: 1, lon: 2 }, species: ['kelp_bass', 'white_seabass'], days: [{ date: '2026-09-09', periods: [period], solunar: { majorPeriods: [], minorPeriods: [] } }], tidePredictions: [] }, units: { temperature: '°C', windSpeed: 'kt', waveHeight: 'ft', wavePeriod: 's', height: 'ft' }, loading: false, error: null, refetch: vi.fn() }),
...
describe('FishingTab presentation', () => {
  it('uses scrollable species and period-card strips instead of a primary forecast table', () => {
    ...
    expect(getByTestId('fishing-period-cards').querySelectorAll('article')).toHaveLength(1);
