...
export interface SurfForecast {
   ...
   * renderer — no client-side eligibility re-derivation. All four nullable/
   * optional: absent on pre-C1 cached responses, in which case the card
   * keeps its pre-C1 rendering (single swellHeight / period value). PA2
   * (MARINE-PAGE-FIXIT-PLAN Phase S, 2026-08-10/11) retired the original
   * `faceHeightMinFt`/`faceHeightMaxFt` pair (Breaking Face Height rebinds
   * to `modelSurfHeightMin`/`modelSurfHeightMax` above) and `combinedPeriodS`
   * (replaced by the `periodMinS`/`periodMaxS` range below).
   */
  ...
  /**
   * PA2 (ruled 2026-08-10, MARINE-PAGE-FIXIT-PLAN Phase S). Lowest peak
   * period (s) across the ELIGIBLE surfable swell components (same
   * eligibility rule as {@link swellHeightMinFt}/{@link swellHeightMaxFt}).
   * Pair with {@link periodMaxS}; render "min–max s" (equal values collapse
   * to one number, same rule as the height ranges). Replaces the retired
   * energy-weighted `combinedPeriodS` — periods of separate wave trains do
   * not combine into a meaningful average (operator ruling, verbatim: "the
   * period should never be combining periods, that is not how the physics
   * works"). `null` when no deep-water reference exists for this timestep.
   */
  periodMinS?: number | null;
  /** Highest peak period (s) across the ELIGIBLE surfable swell components. See {@link periodMinS}. */
