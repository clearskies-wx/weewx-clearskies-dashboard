/**
 * SurfingIcon — inline SVG for the Surfing marine activity icon.
 *
 * Source: material-symbols:surfing (Apache-2.0 — Google Material Symbols)
 * https://fonts.google.com/icons?icon.query=surfing
 * viewBox 0 -960 960 960, 960-unit coordinate system, 24×24 display.
 *
 * DASHBOARD-MANUAL §12 / ADR-049/050: Phosphor has no surfing glyph; this
 * Material Symbols variant is the designated cross-pack exception for the
 * Marine Activities "Surfing" tab/accordion icon, matching the existing
 * inline-SVG cross-pack pattern used for alert icons (earthquake, volcano,
 * tsunami, etc. in src/components/icons/).
 *
 * A11y (rules/coding.md §5.5):
 *   - Decorative by default (aria-hidden) — used alongside a visible text
 *     label (activity name) in tab/accordion headers, so the icon itself
 *     does not need to carry the accessible name.
 */

import type { SVGProps } from 'react';

interface SurfingIconProps extends SVGProps<SVGSVGElement> {
  /** Icon size in px (uniform width = height). Default: 20. */
  size?: number;
}

export function SurfingIcon({ size = 20, ...props }: SurfingIconProps) {
  return (
    <svg
