/**
 * Tsunami — inline SVG for the tsunami alert icon.
 *
 * Source: mdi:tsunami (Apache-2.0 — Material Design Icons / Pictogrammers)
 * https://pictogrammers.com/library/mdi/icon/tsunami/
 * viewBox 0 0 24 24, filled path.
 *
 * ADR-050: carbon:tsunami was the first-choice glyph. On visual review the
 * Carbon version (32×32, detailed realistic wave) sits noticeably heavy
 * against Phosphor regular-weight icons. The pre-vetted fallback mdi:tsunami
 * (24×24, simpler filled path, closer weight character) was chosen instead.
 * Brief explicitly permits this swap: "if it looks off against Phosphor weight
 * … use the pre-vetted mdi:tsunami — note which."
 *
 ...
 *     role="alert" / role="status" wrapper and visible text label (ADR-050 §5.1).
 */
...
export function Tsunami({ size = 20, ...props }: TsunamiIconProps) {
  return (
    <svg
      ...
      {...props}
    >
      {/* mdi:tsunami — wave curling over with surfer silhouette */}
